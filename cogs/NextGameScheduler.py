...
from soulbot_support import CT, ET, GMT, MT, PT, soulbot_db
...
class NextGameScheduler(commands.Cog, name="Next Game Scheduler"):
    ...
    async def schedule(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send(
                f'Additional arguments requires, see **{ctx.prefix}help next schedule** '
    ...
    async def default(self, ctx):
        ...
        """
        default_date = datetime.now().replace(
    ...
    async def set_time(self, ctx, schedule_time: str, schedule_tz: str):
        ...
        if (sch_hour > 24) or (sch_minute > 59):
            ...
            next_game_scheduled = soulbot_db.next_game_db_get()
            output_date = (
                next_game_scheduled[0]
                .replace(
                    hour=sch_hour, minute=sch_minute, microsecond=0, second=0, tzinfo=time_zone
                )
                .astimezone(GMT)
            )
            soulbot_db.next_game_db_add(output_date)
