                ...
                for e in exits:
                    ...
                    while not exit_complete:
                        ...
                        if room in self.dungeon[destination_room]["exits"]:
                            ...
                            destination_room = random.choice(room_list)
                        if len(self.dungeon[room]["exits"]) < 4 and len(self.dungeon[destination_room]["exits"]) < 2:
...
class DungeonGen(commands.Cog, name="Dungeon Generator"):
    ...
    @dg.commandO(help="Enter the number of the direction to go.")
    async def go(self, direction: int):
        directions = dungeongen_obj[ctx.guild.id].dungeon[dungeongen_obj[ctx.guild.id].current_room_id]
