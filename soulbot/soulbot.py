...
loguru.logger.add(LOG_LOCATION, rotation="1 MB", retention="30 days")
...
class SoulBot(commands.Bot):
    ...
    def __init__(self, config: Dict[str, Any], guild_init: Dict | None = None):
        ...
        self.prefixes = soulbot_support.soulbot_db.config_all_prefix_load()

        intents = discord.Intents.default()
        intents.members = True
        intents.message_content = True
        loguru.logger.info(
            f"Initializing SoulBot with config: {config}\nIntents: {dict(intents)}"
        )
...
class ConfigCommands(commands.Cog):
    ...
    async def config(self, ctx: commands.Context) -> None:
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send(
                f"Additional arguments required, see **{ctx.prefix}help config** for available options."
            )

    @config.command(
        help="Display the current guild config options.", name="display"
    )
    async def display_config(self, ctx: commands.Context) -> None:
        try:
            if not ctx.guild:
                ...
                return
            config = soulbot_support.soulbot_db.config_get_guild(ctx.guild.id)
            ...
            await ctx.send(
                f"Current configuration for {ctx.guild.name}:\n{output}"
            )
        except (Exception, RuntimeError) as e:
            await ctx.send(
                f"An error occurred while displaying the configuration: {e}"
            )
    ...
    async def set_prefix(self, ctx: commands.Context, prefix: str) -> None:
        ...
        soulbot_support.soulbot_db.config_prefix_update(ctx.guild.id, prefix)
        await ctx.send(f"Prefix now set to {prefix}.")

    @config.command(help="Set Next Game default start time.", name="time")
    async def next_game_time(
        self, ctx: commands.Context, default_time: str, default_timezone: str
    ) -> None:
        ...
        validation_result = self._validate_time_input(
            default_time, default_timezone
        )
        ...
        soulbot_support.soulbot_db.config_next_game_default_time_update(
            ctx.guild.id, time_output
        )
        await ctx.send(
            f"NextGameScheduler default game start time is now set to {time_output}."
        )

    @config.command(
        help="Set Next Game default game interval in days.", name="interval"
    )
    async def next_game_interval(
        self, ctx: commands.Context, default_interval: int
    ) -> None:
        ...
        soulbot_support.soulbot_db.config_next_game_default_interval_update(
            ctx.guild.id, default_interval
        )
        await ctx.send(
            f"NextGameScheduler default game interval is now set to {default_interval} days."
        )
    ...
    async def next_game_announce_channel(
        ...
        if channel_name not in valid_channels:
            await ctx.send(
                f"{channel_name} is not a valid text channel. Please try again."
            )
            return

        soulbot_support.soulbot_db.config_next_game_announce_channel(
            ctx.guild.id, channel_name
        )
        await ctx.send(
            f"Next Game Scheduler announcements will be sent to {channel_name}."
        )

    def _validate_time_input(
        self, time_str: str, timezone_str: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            ):  # Fixed: hour should be 0-23, not 0-24
                return {
                    "error": True,
                    "message": "Please use 24 hour time in the format: HH:MM TZ(Eg: 19:00 ET)",
                }

            if timezone_str.upper() not in VALID_TIMEZONES:
                return {
                    "error": True,
                    "message": "Please indicate your timezone: ET, CT, MT, or PT.",
                }
    ...
    async def config_error(
        ...
        if isinstance(error, commands.BadArgument):
            await ctx.send(
                "Please use numeric values only for the game interval."
            )
        elif isinstance(error, commands.MissingPermissions):
            await ctx.send(str(error))
        else:
            loguru.logger.error(f"Config command error: {error}")
...
class GameAnnouncer:
    ...
    async def game_announce_task(self) -> None:
        ...
        try:
            announce_check = (
                soulbot_support.soulbot_db.next_game_get_all_announcing()
            )
            ...
            loguru.logger.error(f"Error in game announcement task: {e}")

    async def _process_server_announcement(
        self, server_data: soulbot_support.NextGame
    ) -> None:
        """Process announcement for a single server."""

        guild = self.bot.get_guild(server_data.guild_id)
        if not guild:
            loguru.logger.warning(f"Guild {server_data.guild_id} not found")
            return

        config = soulbot_support.soulbot_db.config_get_guild(guild.id)
        channel = discord.utils.get(
            guild.text_channels, name=config.announce_channel
        )
        if not channel:
    ...
    async def _send_announcement(
        ...
        soulbot_support.soulbot_db.next_game_announce_toggle(False, guild_id)
        ...
        if minutes_remaining > 0:
            ...
            await channel.send(
                "@here Next game is starting now!\nFurther announcements have been disabled."
            )
...
def load_config() -> Dict[str, Any]:
    ...
    try:
        ...
        loguru.logger.error(f"Invalid JSON in configuration file: {e}")
        raise
...
def setup_bot(config: Dict[str, Any]) -> SoulBot:
    ...
    async def on_guild_join(guild: discord.Guild) -> None:
        ...
        soulbot_support.soulbot_db.config_insert_all(
            guild.id,
        ...
        bot.prefixes = soulbot_support.soulbot_db.config_all_prefix_load()
        bot.guild_init[guild.id] = init_support.InitiativeTrack()
        loguru.logger.info(f"Joined guild: {guild.name} ({guild.id})")

    @bot.event
    async def on_guild_remove(guild: discord.Guild) -> None:
        """Handle bot being removed from a guild."""
        soulbot_support.soulbot_db.guild_remove_all(guild.id)
        _ = bot.guild_init.pop(guild.id)
        _ = bot.prefixes.pop(guild.id)
        loguru.logger.info(f"Left guild: {guild.name} ({guild.id})")
    ...
    async def on_ready() -> None:
        ...
        for guild in guild_list:
            init_tracker = soulbot_support.soulbot_db.init_db_rebuild(guild)
            if init_tracker is None:
                bot.guild_init[guild] = init_support.InitiativeTrack()
            else:
...
def load_extensions(bot: SoulBot) -> None:
    ...
    try:
        ...
        loguru.logger.info("Loaded DiceRoller extension")
    except Exception as e:
        loguru.logger.error(f"Failed to load DiceRoller: {e}")
    ...
    for cog in bot.config.get("load_cogs", []):
        try:
            ...
            loguru.logger.error(f"Extension not loaded for {cog}: {e}")
        except discord.ExtensionFailed as e:
            loguru.logger.error(f"Extension failed for {cog}: {e}")
        except Exception as e:
            loguru.logger.error(f"Unexpected error loading {cog}: {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        loguru.logger.error(f"Bot startup error: {e}")
