...
class DatabaseIO:
    ...
    def quote_db_random(self, guild_id: int):
        ...
        if count > 0:
            self.c.execute(
                """SELECT id, quote FROM quotes where guild_id=? ORDER BY RANDOM() LIMIT 1""",
                (guild_id,),
            )
    ...
    def next_game_db_add(self, output_date, guild_id: int):
        ...
        self.c.execute(
            ...
            (guild_id, arrow.now(UTC).int_timestamp, output_date),
        )
        self.bot_db.commit()
    ...
    def next_game_announce_toggle(self, state: int, guild_id: int):
        ...
        """
        self.c.execute(
            """UPDATE next_game SET announce_on=? where guild_id=?""",
            (
                state,
                guild_id,
            ),
    ...
    def next_game_get_defaults(self, guild_id: int):
        ...
        """
        self.c.execute(
            """SELECT next_game_start, next_game_interval FROM config where guild_id=?""",
            (guild_id,),
        )
        return self.c.fetchone()
    ...
    def config_insert_all(
        ...
        self.c.execute(
            """INSERT INTO config(guild_id, prefix, next_game_start, next_game_interval, announce_channel)
        VALUES(?,?,?,?,?)""",
            (guild_id, prefix, default_time, default_interval, announce_channel),
        )
        self.bot_db.commit()
    ...
    def config_prefix_update(self, guild_id: int, prefix: str):
        ...
        self.c.execute(
            """UPDATE config SET prefix=? WHERE guild_id=?""",
            (
                prefix,
                guild_id,
            ),
    ...
    def config_next_game_default_time_update(self, guild_id: int, default_time: int):
        ...
        self.c.execute(
            """UPDATE config SET next_game_start=? WHERE guild_id=?""",
            (
                default_time,
                guild_id,
            ),
    ...
    def config_next_game_default_interval_update(self, guild_id: int, default_interval: int):
        ...
        self.c.execute(
            """UPDATE config SET next_game_interval=? WHERE guild_id=?""",
            (
                default_interval,
                guild_id,
            ),
    ...
    def config_next_game_announce_channel(self, guild_id: int, announce_channel: str):
        ...
        self.c.execute(
            """UPDATE config SET announce_channel=? WHERE guild_id=?""",
            (
                announce_channel,
                guild_id,
            ),
